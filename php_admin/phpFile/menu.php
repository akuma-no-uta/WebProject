...
// Lấy dữ liệu menu có giảm giá
$menus = pdo_query("
    SELECT m.*, c.name AS category_name, d.giam_phan_tram AS phan_tram_giam
    FROM menu m
    LEFT JOIN category c ON m.phan_loai = c.id
...
<table border="1" cellpadding="10">
    ...
    <tbody>
        <?php foreach ($menus as $menu): ?>
            <?php
                $giaGoc = $menu['gia'];
                $giam = isset($menu['phan_tram_giam']) ? $menu['phan_tram_giam'] : 0;
            ...
            <tr>
                ...
                <td>
                    <?php if ($giam > 0): ?>
                        <span style="text-decoration: line-through; color: gray;"><?= number_format($giaGoc, 0) ?> VND</span><br>
                        <span style="color: red; font-weight: bold;"><?= number_format($giaSauGiam, 0) ?> VND (-<?= $giam ?>%)</span>
                    <?php else: ?>
